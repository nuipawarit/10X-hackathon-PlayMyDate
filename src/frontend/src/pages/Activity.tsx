...
import { startActivity, completeActivity, getAvailableActivities } from '../services/api';
...
export default function ActivityPage() {
  const { matchId, activityId } = useParams<{ matchId: string; activityId: string }>();
  ...
  const loadActivity = async () => {
    try {
      const response = await getAvailableActivities(matchId!);
  ...
  if (!activity) {
    return (
      <div className="card text-center py-16 animate-fade-in">
        ...
        <button onClick={() => navigate(`/matches/${matchId}`)} className="btn-secondary">
          Back to Chat
        </button>
      </div>
    );
